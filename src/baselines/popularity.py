...
def score_candidates(candidates_df, train_df, item2idx):
    item_pop = train_df.groupby("i").size().sort_values(ascending=False).to_dict()
    cand = candidates_df.copy()
    cand["i"] = cand["edition_id"].map(item2idx).astype(int)
    cand["score"] = cand["i"].map(lambda x: item_pop.get(x, 0)).astype(float)
...
def main():
    ...
    ap.add_argument("--out", default="submission_popularity.csv")
    ...
    cand_scored = score_candidates(ds.candidates, split.train_df, maps.item2idx)
    ...
    if split.val_df is not None and len(split.val_df) > 0:
        ...
        val_scored = score_candidates(val_candidates, split.train_df, maps.item2idx)
