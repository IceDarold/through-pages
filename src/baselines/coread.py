...
def score_candidates(candidates_df, train_df, user2idx, item2idx, max_seed_items=5):
...
def main():
    ...
    ap.add_argument("--out", default="submission_coread.csv")
    ap.add_argument("--do-local-split", action="store_true")
    ap.add_argument("--max-seed-items", type=int, default=5)
    ...
    cand_scored = score_candidates(ds.candidates, split.train_df, maps.user2idx, maps.item2idx, max_seed_items=args.max_seed_items)
    ...
    if split.val_df is not None and len(split.val_df) > 0:
        ...
        val_scored = score_candidates(val_candidates, split.train_df, maps.user2idx, maps.item2idx, max_seed_items=args.max_seed_items)
